import type { IQuestionEngine, NextQuestionResult } from '@ports/services/IQuestionEngine.js';
...
/**
 * Adapter: Implements IQuestionEngine using conditional logic + heuristic deepening.
 * No Prisma dependency.
 */
export class ConditionalQuestionEngine implements IQuestionEngine {
  selectNextQuestion(
    schemaJson: unknown,
    currentAnswers: Record<string, unknown>
  ): NextQuestionResult {
    const schema = schemaJson as TemplateSchemaJson;
    const answered = getAnsweredIds(currentAnswers);
